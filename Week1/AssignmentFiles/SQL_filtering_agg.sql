...
SELECT DATE(order_datetime) AS order_date, COUNT(*) AS orders_count FROM orders GROUP BY DATE(order_datetime);
-- Q4) What is the average number of items per PAID order?
--     Use a subquery or CTE over order_items filtered by order_id IN (...).
SELECT AVG(order_items.quantity) FROM order_items
...
FROM (SELECT DAYNAME(order_datetime) AS day_name,
	...
	WHERE status = 'paid'
	GROUP BY day_name) x
WHERE orders_count = (SELECT MAX(orders_count) FROM (SELECT DAYNAME(order_datetime) AS day_name,
    COUNT(*) AS orders_count
FROM orders
WHERE status = 'paid'
GROUP BY day_name) y )
...
select date(order_datetime) as order_date, count(order_id)
from orders
